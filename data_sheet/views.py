...
# from django.db.models.functions import ExtractMonth, ExtractDay, ExtractHour, ExtractWeekDay
# import json
# import csv
# import datetime
...
# # -------------------------------
# # View import CSV qua HTTP POST
# # -------------------------------
# @require_POST
# @csrf_exempt
# def import_csv_view(request):
#     """
#     Import CSV từ server (file path cứng hoặc upload)
#     """
#     try:
#         csv_file_path = 'data_ggsheet.csv'
#         if not os.path.exists(csv_file_path):
#             return HttpResponse(f"File không tồn tại: {csv_file_path}", status=400)
...
#                         'segment_code': row.get('Mô tả Phân Khúc Khách hàng', '')
#                     }
#                 )
...
#                 group, _ = ProductGroup.objects.get_or_create(
#                     group_code=row['Mã nhóm hàng'],
#                     defaults={'group_name': row.get('Tên nhóm hàng', '')}
#                 )
...
#                         'group': group,
#                         'unit_price': int(row.get('Đơn giá', 0))
#                     }
#                 )
...
#                     order=order,
#                     product=product,
#                     defaults={'quantity': int(row.get('SL', 1))}
#                 )

#         return HttpResponse(f"✅ Import dữ liệu từ {csv_file_path} thành công!")
#     except Exception as e:
#         return HttpResponse(f"Lỗi import CSV: {str(e)}", status=500)

# # -------------------------------
# # View hiển thị dữ liệu JSON cho D3.js
# # -------------------------------
# def data_visualization(request):
#     """
#     Trả dữ liệu JSON cho template visualization.html
#     """
#     try:
#         # Lấy toàn bộ OrderDetail với thông tin Product & Group
#         order_details = OrderDetail.objects.select_related('order', 'product__group').values(
#             'product__group__group_code',
#             'product__group__group_name',
#             'product__product_code',
...
#         # Dữ liệu Q1: tổng theo sản phẩm
#         data_q1 = [
#             {
#                 'Mã nhóm hàng': od['product__group__group_code'],
#                 'Tên nhóm hàng': od['product__group__group_name'],
#                 'Mã mặt hàng': od['product__product_code'],
#                 'Tên mặt hàng': od['product__name'],
#                 'SL': od['quantity'],
#                 'Thành tiền': od['quantity'] * od['product__unit_price']
#             }
#             for od in order_details
#         ]

#         # Dữ liệu Q2: tổng theo nhóm hàng
#         agg_group = OrderDetail.objects.values(
#             'product__group__group_code',
#             'product__group__group_name'
#         ).annotate(
#             SL=Sum('quantity'),
...
#         data_q2 = [
#             {
#                 'Mã nhóm hàng': g['product__group__group_code'],
#                 'Tên nhóm hàng': g['product__group__group_name'],
#                 'SL': g['SL'],
#                 'Thành tiền': g['Thành_tiền']
#             }
#             for g in agg_group
#         ]

#         # Dữ liệu theo tháng
#         month_data = OrderDetail.objects.annotate(
#             month=ExtractMonth('order__order_time')
...
#         )
#         data_month = [
#             {'Tháng': m['month'], 'SL': m['SL'], 'Thành tiền': m['Thành_tiền']}
#             for m in month_data
#         ]

#         # Trả về template với dữ liệu JSON
#         return render(request, 'visualization.html', {
#             'data_q1': json.dumps(data_q1, default=str),
#             'data_q2': json.dumps(data_q2, default=str),
#             'data_month': json.dumps(data_month, default=str)
#         })
